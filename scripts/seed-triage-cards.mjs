...
patch('measles-us-2026', {
  ...
  initialActions: [
    ...
    'Evaluate vitamin A supplementation for hospitalized children per current CDC/AAP/facility protocol; verify dosing from the linked source before administration',
  ],
  notify: [
  ...
  treatmentSummary:
    'No specific antiviral therapy. Supportive care includes fever control, hydration, and complication monitoring. ' +
    'For hospitalized children, evaluate vitamin A supplementation using current CDC/AAP/facility protocol rather than this card. ' +
    'Post-exposure prophylaxis for susceptible contacts may include MMR or immune globulin when eligible; verify timing, ' +
    'eligibility, contraindications, and dosing with public health and the linked source before administration.',
  sourceAuthority: 'CDC',
...
patch('avian-influenza-h5-2026', {
  ...
  initialActions: [
    ...
    'Initiate empiric antiviral treatment per current CDC/facility protocol without waiting for confirmation if illness is compatible; verify dosing from the linked source before administration',
  ...
  treatmentSummary:
    'CDC recommends prompt empiric antiviral treatment for suspected or confirmed human H5 infection without waiting for confirmatory testing. ' +
    'Dosing, duration, pediatric adjustment, renal adjustment, and post-exposure prophylaxis decisions must be verified against current CDC guidance ' +
    'and local protocol before administration. Severe cases require aggressive supportive care and early consultation with infectious disease, ' +
    'critical care, and public health; ARDS may require ECMO-capable transfer.',
  sourceAuthority: 'CDC',
...
patch('lassa-fever-2026', {
  ...
  treatmentSummary:
    'Ribavirin is the established treatment for suspected severe Lassa fever when started early, but dosing, route, contraindications, pregnancy risk-benefit, ' +
    'monitoring, and post-exposure prophylaxis decisions must be verified through CDC/state public health consultation and current facility protocol before administration. ' +
    'Aggressive supportive care includes fluid and electrolyte management, blood products for coagulopathy, renal support when indicated, and early transfer planning ' +
    'for severe disease. No licensed Lassa vaccine.',
  sourceAuthority: 'CDC',
