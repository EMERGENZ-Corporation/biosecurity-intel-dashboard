...
Extended live host-city monitoring to Canada and made the Mexican cities' unmonitored state explicit. User question was "is there a reason we can't get Canada and Mexico working?" — investigated and found: **Canada is doable** (PHAC publishes machine-readable CSVs), **Mexico genuinely is not** (no centralized, machine-readable national wastewater feed exists). Took coverage from 11/16 → 13/16 cities, with the remaining 3 (Mexico) now honestly labelled.
...
**Verify:** `npm run test:ingest-phac && npm run test:validators && npm run validate:data && npm run build` (PASS). To go live: merge to main + trigger `Ingest PHAC Host-City Wastewater` (`workflow_dispatch`); Toronto/Vancouver respiratory tiles then show real SARS-CoV-2 wastewater status. **Backlog:** PHAC influenza/RSV ingestion (separate feed); optional metro-county scoping for NWSS rollup.
