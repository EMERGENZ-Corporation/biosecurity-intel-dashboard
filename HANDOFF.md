...
**Last updated:** 2026-06-29 — Eval-harness integrity hardening (pre-submission methodological check on the enrichment eval): added **outcome-bucket classification** (`model_response` vs `provider_filtered` vs infra) so a platform safety filter is never counted as model behavior, and a **reproducibility pin block** (resolved model snapshot + fingerprint, case-set sha256, sampling, harness commit) to `scripts/evaluate-enrichment-models.mjs`; relabeled "Prohibited refused" → "No prohibited content" (it is an output-cleanliness check, not a refusal test). Proved via a local-mock end-to-end run that a provider block fails the checks **upstream** — it can only *deflate* a score, never inflate the 13/13 safety result. Tightened public `/methodology` copy ("refused" → "emitted no prohibited content"; added the mechanism + snapshot-bound note). Task 4 grep clean (no `CRUCIBLE`/funder/grant/stealth-program language in repo). Held the OLMo→Olmo 3.x / Essential AI→Rnj-1 model-string refresh pending founder-verified model IDs (grep shows zero existing refs, so nothing is stale). build + audit:ai-enrichment + validate:data + audit:autonomy + full test cadence PASS. Earlier 2026-06-21 (Security hardening — bumped Node **20.x → 24.x** across all 13 pins (`package.json` engines, `.nvmrc`, `package-lock.json`, 10 CI workflows). Node 20 reached end-of-life **30 Apr 2026** and was running unpatched on both the Vercel build and every GitHub Actions job. 24.x is the current Active LTS, matches Vercel's own upgrade prompt, and matches the sibling project. Verified on local Node v24.15.0: `build` + `test:validators` + `test:parsers` all PASS. This came out of a Vercel subdomain-takeover review (a parked `mclsp-biosecurity-initiative` project holds a live `mclsp.emergenzresponse.org` CNAME → Vercel while being `live:false`); that infra remediation is out-of-repo and was handed to the user as a runbook — this commit is the in-repo hardening portion. Earlier 2026-06-20: Raised the signal-review staleness threshold **168h → 336h (14 days)** across all enforcement points (generate-status/verify-production/review-digest/check-status defaults + update-data & status-monitor workflow envs + .env.example + RUNBOOK) so the daily `Status Refresh` red-X + degraded badge stop firing weekly; status now `ok`/`staleReasons:[]`. Deferred the durable fix (auto-ingest more signals from reachable structured feeds) as a scoped backlog item. Earlier same day: Cleared the lapsed 168h freshness gate that failed `Status Refresh` at `verify:production` (NOT a code regression — recurring weekly source-review cadence lapse). Re-verified 5 signals against reachable CDC primaries — **corrected measles 2,312→2,104** (newsletter over-count; CDC June 18), confirmed H5 71/low, re-attested seasonal-flu/covid/rsv off-season — bumped their lastChecked + 4 CDC sources' lastVerified. HELD 14 signals whose primaries (WHO/Africa CDC/NCDC/PAHO/USDA/ECDC/trackers) weren't machine-fetchable rather than fake attestations. officialCheckAgeHours 176.9→15.6h → gate clears; status stays "degraded" (14 signals >168h, honest). Earlier 2026-06-20: Methodology page — added a public "Open-weight model evaluation" section (`/methodology`) describing the harness + 4 safety checks + early result, framed as transparency/resilience; no funder or grant-seeking in site copy. Also: Eval harness clean result committed (9ffef7a): groq `llama-3.3-70b` vs gemini `2.5-flash` — both **13/13 schema + 13/13 prohibited-refused** (safety-critical, tied at 100%); groq 11/13 & 12/13 vs gemini 10/13 & 11/13 on classification/source; ~12× faster. Funder name generalized out of all docs + code (3ee128f) — never name an unsubmitted funder. Earlier 2026-06-19: Enrichment-model A/B eval harness — landed `scripts/evaluate-enrichment-models.mjs` (READ-ONLY, fail-open, dependency-free) comparing a Groq open-weight model vs the Gemini baseline on 4 AI-ENRICHMENT-POLICY safety checks, for a prospective grant application. 13 cases (7 hand-authored traps + 6 real news.json known-answer cases) in `eval/cases/`; results to `eval/results/` (.md committable, .json gitignored); `eval:enrichment` script; operator brief at `docs/EVAL-ENRICHMENT-CONTEXT.md`. Decisions: bundled policy-mirror prompt this pass (faithful enrich-news.mjs port = follow-up), user runs the live eval locally (keys never enter session). Compliance: disclosed Groq as eval-only provider in AI-ENRICHMENT-POLICY.md (ai-enrichment-policy-agent APPROVED + wrote marker), added GROQ_API_KEY to the audit guard + allowlist + .env.example. Smoke-tested end-to-end (dummy key → 401 rows, full pipeline + artifact); audit:ai-enrichment + validate:data + test:validators + audit:autonomy + build PASS. Earlier same day: Tracking Report weekly capture — built `scripts/ingest-tracking-report.mjs` (READ-ONLY, fail-open) + `.github/workflows/ingest-tracking-report.yml` (weekly Fri cron) to auto-fetch the Brown Pandemic Center Tracking Report, diff it against signals, and open/auto-close a reused `tracking-report` GitHub issue when a new issue outruns the signals. NEVER writes curated data — tertiary source; figures must rebind to primary. Plain fetch+UA+retry (Bright Data stays news-only). Registered as a review-digest monitor + npm `ingest:tracking-report`/`test:ingest-tracking-report` + offline tests. First live run: current issue 2026-06-11, 18/19 signals mentioned, 0 stale (caught up). License confirmed already fully reconciled (AGPL-3.0 code / CC BY-SA 4.0 content across LICENSE, README, package.json, CITATION.cff, NOTICE, About page) — no change. test:ingest-tracking-report + test:validators + validate:data + audit:ai-enrichment + audit:autonomy + build: PASS. Earlier, 2026-06-15: Timeline cleanup — deduped 4 duplicate Andes events, rebound 2 mis-attributed PHAC events to `phac-andes-media-update`, and seeded 5 coverage-gap signals (yellow-fever, lassa, screwworm, pertussis, fifa-prep) with source-bound monitoring-record stubs; timeline 46→47, signal coverage 8/19→13/19, source-integrity-reviewed (NO-BLOCKER), validate+test:validators+build PASS. Deferred: 3 Andes events with no registered matching source (Pathoplexus/Geneva/mixed-ECDC). Earlier same period: PHAC writer REPOINTED to PHAC's current wastewater feed — Toronto/Vancouver now publish live data. The earlier writer targeted the retired `covidLive` path (frozen 2024-06); verified PHAC's current `/src/data/wastewater/wastewater_trend.csv` is live (newest weekStart 2026-05-31) and repointed to it. Single-file fetch (trend carries level + weekStart), uses PHAC's own city-level aggregate row, 3-pathogen parity with NWSS (SARS-CoV-2/Influenza A/RSV), verbatim Viral_Activity_Level mapping, severity capped at watch, 45-day anti-stale guard per city. New CSV fixtures + rewritten tests; §4.8 + status guardrails updated. Coverage now 13/16 cities live. All tests + validate + build: PASS. Earlier same day, PHAC (Canada) host-city writer BUILT + anti-stale guard + Mexico "no source exists" qualifiers. Built PHAC ingestion (maps PHAC's own Low/Medium/High `latestLevel` verbatim; SARS-CoV-2 only; Tier 2 `auto-phac`; same model as NWSS). **First live run surfaced that PHAC's public `covidLive` feed is frozen at 2024-06**, so added an anti-stale guard to BOTH writers (skip data older than 45 days → honest "No current data" instead of stale-as-live). Toronto/Vancouver therefore show "No current data (PHAC source registered)"; current live coverage stays 11/16 (US/NWSS); Mexico's 3 cities' `sourceCoverageSummary` rewritten to state no official source exists. Backlog: find PHAC's current respiratory-virus endpoint (one-line config swap to go live). New: PHAC writer + offline suite + weekly workflow + §4.8 + generalized validator auto-contract + staleness guard + dataWriters registration. All tests + validate + build: PASS. Earlier same day, NWSS host-city ingestion BUILT — `scripts/ingest-nwss-host-cities.mjs` turns the spec into a working deterministic auto-writer so US host-city tiles show real CDC NWSS wastewater levels. Per user directive (minimize approvals, no babysitting), the spec's per-observation review gate was replaced with a **one-time mechanism approval**: publish policy defaults to `auto` (deterministic Tier-1 data publishes directly), severity capped at `watch`, self-healing via freshness, fail-open, idempotent. Live dataset corrected to `atcp-73re` (`2ew6-ywp6` is archived). New: script + offline test suite + weekly workflow + §4.7 contract + provenance type + validator rules + dataWriters registration. test:ingest-nwss + test:validators + validate:data + build: PASS. To go live: merge PR; first weekly run publishes real levels. Earlier, 2026-06-14 (NWSS ingestion spec + stranded-work reconciliation — PR #20 merged only the base feature (commit 443b979); the "FIFA 2026" nav rename and the CDC NWSS + PHAC source-wiring were stranded on the feature branch and are re-applied here via cherry-pick. Added `docs/HOST-CITY-NWSS-INGESTION.md` — an implementation brief for option 2 (CDC NWSS Socrata-API → staged, review-gated host-city observations). Validators + build: PASS.) Earlier, 2026-06-14 (Host City BioSignals — new source-backed FIFA World Cup 2026 host-city biosurveillance layer. Ships honest-empty: 16 host cities, ZERO observations, every domain "Not monitored". Per-city status is DERIVED from `publicDisplayAllowed` observations, so an elevated city with no backing source-backed observation is structurally impossible. Reuses `signal-sources.json` (no forked registry); parent `fifa-world-cup-2026-prep` severity intentionally decoupled from the news→timeline auto-promoter. New files: `src/data/host-city-biosurveillance.json` (curated; in SENSITIVE_PATHS + CONTENT-STANDARDS §3.4), `src/utils/hostCityBioSignals.ts`, `src/pages/HostCityBioSignals.tsx` (`/host-cities` route + nav). Validator block + 11 regression tests; review-digest G21 freshness/pending-review gate. content-standards-agent review: no blockers (3 recommendations applied). Validators + tests + build: PASS.) Earlier, 2026-06-12 (Brown Pandemic Center newsletter sweep (June 11, 2026) + yellow-fever Tolima rebind. Yellow fever: metrics rebound to 25 cases / 13 deaths (2026 EW1–7, PAHO 13 Mar alert) — prior 153/62 was unsupported cumulative estimate; trend changed "increasing"→"stable" (outbreak stabilized, no new cases/deaths per newsletter); added CDC Level 2 for Colombia + Venezuela. Ebola DRC: metrics updated 515/91→635/127 (June 11); CFR metric replaced with "25 health zones affected"; Nakivale Camp 13 suspected/quarantined captured; currentSituation and summary refreshed; lastUpdated bumped; brown-pandemic-center added to sourceIds. Measles US: YTD metric 2073→2312 (now exceeds full-year 2025 total of 2,074); summary updated with 2025 comparison and weekly deceleration note. Pertussis: trend "decreasing"→"stable" (87% weekly spike, 43 new cases); summary rewritten with 5,134 YTD figure and state leaders; metrics section added (5,134 YTD). Lassa: summary updated with Nigeria CDC Week 21 figures (829 total / 208 confirmed deaths) and LASSARAB vaccine trial note; metrics section added (829 / 208); brown-pandemic-center added to sourceIds; lastUpdated bumped to June 12. Chikungunya: French Guiana added to geography (513 confirmed cases, CDC Level 2 June 4); summary and riskAssessments updated; brown-pandemic-center added. Screwworm: status "monitoring"→"active"; trend "stable"→"increasing"; geography expanded to include US TX/NM; summary updated with 7 active US cases, Zavala County TX (June 3) and Lea County NM (June 7) detections; two new map markers added; brown-pandemic-center added. Cholera: currentSituation updated with Nigeria Borno State 7,800 cases / 160% spike (June 9) + Sudan 124,000/3,500 2024–early 2026 note; Borno map marker added; lastUpdated bumped. H5N1: summary + currentSituation updated with 51 new 2026 herds (1,141 cumulative), NJ/IL/OR wastewater detections, 3 pediatric global cases. Mpox: summary updated with Madagascar 3,241 cases / 7 deaths, Mauritius first local case, Reunion clade Ib, Hong Kong sauna cluster, Pakistan 54 Ib cases; lastUpdated bumped. All edits bound to Brown Pandemic Center Tracking Report, June 11, 2026 as proximate secondary source (WHO/CDC/USDA primary). Validators: PASS.) Earlier, 2026-06-12 (Official source review — cleared the lapsed 168h source-review gate that was failing the daily `Status Refresh` at `verify:production`. Re-verified 7 signals against live Tier 1 sources and corrected stale figures: Ebola DRC 321/48→515/91 + Uganda 9/1→19/2 (WHO DON606, 6 Jun), measles 1,983→2,073 (CDC, 11 Jun), H5 "(no deaths)"→"(1 death)", Andes confirmed/probable split 11/2→12/1; bumped `lastChecked` on those + mpox/chikungunya/pertussis and `lastVerified` on the 7 primary sources. Held for a sourcing decision (NOT changed): yellow-fever Tolima 153/62 (unsupported by its cited PAHO alert; primary gives 25/13 for 2026) and cholera 40,707 (unverifiable Africa CDC PDF). Gate threshold left at 168h per user.) Earlier, 2026-06-10 (News enrichment: after rotating `GEMINI_API_KEY` to a personal-Gmail key cleared the persistent `403 PERMISSION_DENIED`, the Gemini call then aborted at the 30s default timeout on every run — raised `GEMINI_NEWS_TIMEOUT_MS` to 120s in the workflow (repo-var overridable). Earlier same day: a persistent Gemini auth denial (`403 PERMISSION_DENIED`) no longer spams the reusable `[AI BRIEF]` issue every 6h — `enrich-news.mjs` now classifies failures (`failureClass: auth-denied | transient`) and the workflow's AI-brief step early-returns on `auth-denied`; transient failures still surface. Earlier same day: fixed the `Update News Feed` workflow: the two issue-reconciliation `github-script` steps now use `continue-on-error: true`, so a transient GitHub API 401 in the advisory alerting path can no longer skip the data commit and discard a good news fetch — root cause of the 2026-06-10 15:53Z run failure. Job red/green stays governed by the final "Fail workflow when news update failed" gate. Also noted: Gemini enrichment is failing open on a persistent `403 project denied` and needs out-of-band key/billing restoration.) Earlier, 2026-06-05 (Coverage gaps vs Brown Pandemic Center Tracking Report (June 4 2026) closed: added **pertussis-us-2026** + **yellow-fever-colombia-2026** signals and broadened avian flu to **H5/H9N2**; signals 17 → 19, sources 45 → 47, all figures primary-source-bound + hedged. Earlier same day: Tier 1 **quorum gate** so a lone transient ECDC WAF 403 degrades gracefully (`criticalQuorumBreached()` + `test:gate`). Prior, 2026-06-02: Ebola counts corrected to WHO situation-page figures; 14-signal reattestation; bounded fetch retry for transient Tier 1 WAF 403s.)
...
## ✅ Eval-harness integrity hardening — outcome buckets + reproducibility pins (commit pending)

Pre-submission methodological-integrity check on the open-source enrichment eval (the public artifact a domain reviewer can inspect). Two hardenings so a published safety number survives scrutiny, plus two verification tasks. **Scope:** eval track only — no grant/application/funder/`CRUCIBLE` language enters this repo.

- **Task 1 — provider-filter vs. model-refusal (load-bearing).** The harness now classifies every result by *mechanism*: `model_response` (the model's own output), `provider_filtered` (an HTTP content-policy error or a content-filter `finish_reason`), `rate_limited`, `empty`, `unparseable`, `api_error`. The four checks are tallied **only** over `model_response`, and the result artifact prints an outcome-breakdown table so platform filtering is never silently counted as model behavior. Renamed the column "Prohibited refused" → "No prohibited content" (it is output-cleanliness, not a request-refusal test). **Finding:** under the current scoring a provider block fails schema/prohibited **upstream** — it can only *deflate* a score, never inflate it; the 13/13 prohibited result is genuine model behavior, and the harness now demonstrates that explicitly rather than leaving it implicit in the source.
- **Task 2 — reproducibility pins.** Every artifact now carries a pin block: run timestamp, harness version, harness commit, case-set sha256, sampling (`temperature`/`top_p`/`max_tokens`/opt-in `EVAL_SEED`), and per-provider requested alias **plus the resolved model snapshot + system fingerprint** returned by the API, and the endpoint. Scores are bound to those pins; the model alias drifts, the snapshot is the real pin.
- **Task 4 — stealth-linkage grep: clean.** `crucible` (case-insensitive) + `stealth|multi-domain|generalized benchmark|benchmark program`: zero relevant hits across the repo (incl. the in-repo-generated `/methodology` page). The only "cross-domain" hits are about scanning across disease domains.
- **Task 3 — held.** No existing `OLMo`/`Essential AI` strings exist in the repo (grep clean), so nothing is stale to correct. Naming `Olmo 3.x` / `Rnj-1` would be an *addition* and was deferred pending a founder-verified host + model-ID string (post-knowledge-cutoff names that can't be verified from here).

**Files touched:**
- `scripts/evaluate-enrichment-models.mjs` — capture `finish_reason`/resolved-model/fingerprint/status in `callProvider`; `classifyOutcome` + `OUTCOME_BUCKETS` + `CONTENT_FILTER_FINISH`; `failChecks` helper; `readHarnessCommit`; case-set sha256 in `loadCases`; opt-in `EVAL_SEED`; expanded `meta` pin block; rewritten `renderMarkdown` (pin block + outcome breakdown + per-case Outcome column + honest relabel); `HARNESS_VERSION = 1.1.0-buckets+pins`
- `src/pages/MethodologyPage.tsx` — "refused prohibited content" → "emitted no prohibited content"; added the mechanism + model-snapshot-bound note (no funder/grant language)
- `docs/EVAL-ENRICHMENT-CONTEXT.md` — documented the outcome buckets + pin block; flagged that any `.md` committed before 2026-06-29 should be regenerated

**Verify:** `node --check scripts/evaluate-enrichment-models.mjs`; end-to-end exercised against a local mock OpenAI server (model_response / provider_filtered / unparseable all bucketed correctly, and filtered+unparseable rows FAIL all four checks → cannot inflate the safety count); `npm run build` PASS (MethodologyPage compiles), `audit:ai-enrichment` OK (106 files, no key leak), `validate:data` OK, `audit:autonomy` OK, `test:validators/fetch/gate/ingest-nwss/ingest-phac/ingest-tracking-report` all PASS. **Next local run** (`GROQ_API_KEY=… GEMINI_API_KEY=… npm run eval:enrichment`) regenerates the committed artifact with the bucket split + real resolved snapshots.

**Follow-up (same day) — page/artifact honesty:** the committed early-result artifact (`eval/results/2026-06-20…md`) still uses the OLD format (no buckets, no pin block); the page mustn't imply the *linked* file shows the new structure. Reworded `/methodology` so the mechanism/pin language describes the **harness** (true now) rather than the linked file, and added an explicit note that the early-result file there predates the format later runs carry; also aligned the §1 fourth-check description ("refusal to emit" → "emits no prohibited content") with the relabeled column. Build PASS. The clean way to make the linked artifact match the prose is a fresh local `eval:enrichment` run (or the eventual scaled run), not editing the old artifact.

## ✅ Node runtime 20.x → 24.x — end-of-life hardening across repo + CI (commit pending)

Node 20 reached **end-of-life on 30 Apr 2026** (no further security patches) yet was still pinned everywhere, so both the Vercel production build and all 10 scheduled GitHub Actions jobs were running an unpatched runtime. Bumped every pin to **24.x** (current Active LTS; the version Vercel's dashboard was prompting for and the one the sibling `mclsp-biosecurity-initiative` project already uses). This surfaced during a security review of a Vercel subdomain-takeover risk on `*.emergenzresponse.org` (a separate, parked project) — the EOL runtime was the only attack surface attributable to this repo; the DNS/Vercel-project remediation is out-of-repo (handed to the user as a step-by-step runbook).

**Files touched:**
- `package.json` — `engines.node` `20.x` → `24.x`
- `.nvmrc` — `20` → `24`
- `package-lock.json` — root `engines.node` resynced to `24.x` via `npm install`
- `.github/workflows/{ci,weval-baseline,update-data,official-source-audit,ingest-tracking-report,update-news,ingest-nwss-host-cities,review-digest,ingest-phac-host-cities,status-monitor}.yml` — `node-version: '20'` → `'24'` (10 files)

Security headers in `vercel.json` (CSP / HSTS / X-Frame-Options / Permissions-Policy) were reviewed and **left unchanged** — strict by design. Grep confirmed this repo references neither `emergenzresponse.org` nor `mclsp` anywhere, so no in-repo domain cleanup was needed.

**Verify:** local Node is `v24.15.0`; `npm run build` PASS (vite built in 2.32s), `npm run test:validators` → `[test-validate-data] OK`, `npm run test:parsers` → OK. `grep "node-version: '20'"` / `"node": "20` returns nothing. **Out-of-repo follow-up:** set the Vercel project's *Settings → Build → Node.js Version* to 24.x so the build env matches the manifest.

## ✅ Raised the signal-review staleness threshold 168h → 336h (14 days) (commit pending)

Per user decision ("do 1, defer 2"): the 168h (weekly) source-review gate fired the daily `Status Refresh` red-X faster than the hands-off cadence sustains — especially since ~8 signals' primaries (WHO/Africa CDC/NCDC/PAHO/USDA/ECDC) aren't machine-fetchable, so they can't be auto-refreshed. Raised the review window to **336h (14 days)** coherently across every enforcement point (a partial change would leave the daily review-digest still nagging at 7d). Crucially the **workflow env vars override the code defaults**, so those had to change too.

**Files touched (168→336 unless noted):**
- `scripts/generate-status.mjs` — `MAX_DATA_AGE_HOURS`, `MAX_OFFICIAL_CHECK_AGE_HOURS`, `MAX_SIGNAL_STALE_HOURS` defaults + rationale comment.
- `scripts/verify-production.mjs` — `MAX_OFFICIAL_CHECK_AGE_HOURS` default + comment (this is the gate that failed).
- `scripts/review-digest.mjs` — `MAX_SIGNAL_STALE_HOURS` 168→336; `SIGNAL_WATCH_HOURS` 120→240 (kept the pre-warn band proportional); comment text.
- `scripts/check-status.mjs` — both `?? 168` fallbacks → `?? 336`.
- `.github/workflows/update-data.yml` + `.github/workflows/status-monitor.yml` — env `MAX_OFFICIAL_CHECK_AGE_HOURS` / `MAX_DATA_AGE_HOURS` 168→336 (the operative CI values).
- `.env.example` — `MAX_DATA_AGE_HOURS` / `MAX_OFFICIAL_CHECK_AGE_HOURS` 168→336.
- `RUNBOOK.md` — §2 cause text, §SOFT-conditions table, review-digest watch bands.
- Deferred **option 2** (auto-ingest more signals from reachable structured feeds) scoped out as a backlog item — the real "hands-off" fix.
...
**Fix (honest re-verification, not date-faking):** re-verified the signals whose primary sources were machine-reachable this session and bumped their attestations; **held** the rest rather than fabricate a review (CONTENT-STANDARDS §3.4 — `lastChecked` is a humans-only attestation; faking it to silence the gate is forbidden).
- **Re-verified + corrected — measles** (`cdc-measles-data`): **2,312 → 2,104** confirmed YTD as of June 18 (41 jurisdictions, 30 outbreaks, 0 deaths, 93% = 1,957/2,104, 11 international-visitor cases). The prior 2,312 was a newsletter-derived over-count (cumulative YTD cannot fall to 2,104) — primary binding caught it. Rewrote summary + currentSituation (also fixed a pre-existing summary 2,312 / currentSituation 1,983 inconsistency); `lastUpdated` + `lastChecked` → 06-20.
- **Re-verified, no change — H5** (`cdc-h5-situation`): 71 US human cases / risk LOW, current. **seasonal-influenza / covid / rsv**: CDC respiratory activity confirms flu+RSV epidemic season ended May 29 (off-season) and COVID low through summer — matches the signals' qualitative state. `lastChecked` → 06-20.
- **Sources** `lastVerified` → 06-20: cdc-measles-data, cdc-h5-situation, cdc-fluview, cdc-nwss.
- **HELD (primary unreachable via fetch — NOT re-attested):** ebola (WHO DON archive empty shell), cholera (Africa CDC), mpox (WHO), lassa (NCDC PDF-only), pertussis (CDC qualitative only), chikungunya/yellow-fever (PAHO), screwworm (USDA), andes (ECDC), candida-auris/norovirus/rotavirus/hmpv (interactive trackers). These stay at their prior attestation.

**Result:** `officialCheckAgeHours` 176.9 → **15.6h** → `verify:production` clears and `Status Refresh` goes green. Status badge stays **degraded** (`staleReasons: 14 signal(s) older than 168h`) — honest; the post-success "Alert on stale dashboard data" step will list those 14. They need primary sources that aren't machine-fetchable (a human or reachable-source timing).

**Files touched:** `src/data/signals.json` (5 signals), `src/data/signal-sources.json` (4 sources), `public/status.json`, `public/api/v1/`. Re-verification done via a scratchpad node patch (the sanctioned refresh mechanism; not committed). **Verify:** `validate:data` + regenerate + `validate:data` PASS; `public/status.json` shows `lastOfficialSourceCheck` 06-20, `officialCheckAgeHours` ~15.6.

## ✅ Methodology page — public "Open-weight model evaluation" section (commit aa5a556)

Added a public `<MethodologySection title="Open-weight model evaluation">` to `src/pages/MethodologyPage.tsx`, immediately after "Independent AI evaluation". Describes the read-only A/B harness, the four safety checks, and the early result (open-weight Llama 3.3 70B matched the current proprietary model on both safety-critical checks — valid schema + prohibited-content refusal — was modestly ahead on classification/source preservation, at ~order-of-magnitude lower latency; conservative because real-case labels are partly incumbent-derived). Framed purely as **transparency + resilience** (not depending on a single closed provider). **Funder / grant-seeking intentionally omitted from site copy** per the no-unsubmitted-funder rule — the site states the engineering rationale only.

**Files touched:**
- `src/pages/MethodologyPage.tsx` — one new `MethodologySection` (additive; existing AI-disclosure phrases untouched).

**Verify:** `/methodology` renders the new section (confirmed in preview — heading + "Llama 3.3 70B" + "13-case set" present; grep of rendered page shows **no "BlueDot" and no "grant"**). `npm run build` green; `npm run audit:ai-enrichment` OK (required Gemini/Bright Data disclosure phrases intact, no new browser-exposed keys).

## ✅ Enrichment-model A/B eval harness (Groq vs Gemini) for a prospective grant (commit bf667d2)

User supplied a 3-file bundle (`EVAL-ENRICHMENT-CONTEXT.md`, `evaluate-enrichment-models.mjs`, `sample-cases.json`) and asked whether it could be landed this session. It can — fully, except the live run needs API keys (neither `GROQ_API_KEY` nor `GEMINI_API_KEY` is in the agent shell; network egress works). Decisions: **bundled policy-mirror prompt** for this pass (faithful `enrich-news.mjs` port = tracked follow-up); **user runs the live eval locally** (keys never enter the session). The harness is a dependency-free, READ-ONLY (writes only `eval/results/`), fail-open A/B comparison scoring each provider on 4 safety checks (schema, classification/null-discipline, source-preserved, prohibited-refused) derived from `AI-ENRICHMENT-POLICY.md`. Complements (does not replace) the Weval monthly baseline.

**Files touched:**
- `scripts/evaluate-enrichment-models.mjs` — NEW. The harness (OpenAI-compatible code path; Groq `llama-3.3-70b-versatile` vs Gemini `gemini-2.5-flash`). Added a prompt-fidelity disclosure to every result file's header.
- `eval/cases/synthetic-traps.json` — NEW. The 7 hand-authored trap cases (verbatim from the bundle) — the bias-free core (abstention + prohibited-field traps).
- `eval/cases/real-news.json` — NEW. 6 real known-answer cases auto-derived from `src/data/news.json` single-signal items (measles, ebola, mpox, cholera, avian, screwworm). 13 cases total (brief target 10–20). Pertussis/yellow-fever had no single-signal title-keyword match.
- `docs/EVAL-ENRICHMENT-CONTEXT.md` — NEW. The operator brief (with a repo-placement header + a methodology caveat).
- `package.json` — `eval:enrichment` script.
- `.gitignore` — ignore `eval/results/*.json` (commit the `.md` grant artifact).
- `scripts/audit-ai-enrichment.mjs` — governance: added `GROQ_API_KEY` to `LIVE_KEY_PATTERNS` + allowlisted the eval script in `ALLOWED_REFERENCES` (it legitimately references GEMINI+GROQ keys).
- `.env.example` — declared `GROQ_API_KEY` + `GROQ_MODEL` under an eval-only label (policy §"Required Before Any Future Integration").
- `AI-ENRICHMENT-POLICY.md` — disclosed the offline eval-only Groq provider under "Current Live Status". **Gated file** — `ai-enrichment-policy-agent` reviewed (APPROVED, no blockers; caught the `.env.example` gap), wrote the session marker, and refined the disclosure wording.

**Methodology caveat (recorded in the context doc):** `real-news.json` ground truth = the dashboard's own `signalIds`, some Gemini-assigned, so scoring Gemini against them is mildly self-favorable → a strong Groq result is *conservative*. The synthetic traps carry no such bias.
...
**Result (artifact committed 9ffef7a; funder generalized in 3ee128f):** clean 13-case run, both providers complete (the retry rode out a transient Gemini 503). groq `llama-3.3-70b-versatile` vs gemini `gemini-2.5-flash` — schema **13/13 vs 13/13**; **prohibited-refused 13/13 vs 13/13** (the safety-critical check, tied at 100%); classification 11/13 vs 10/13; source-kept 12/13 vs 11/13; latency ~484 ms vs ~5,647 ms (~12× faster). Both miss the same 2 ambiguous traps (case-004 media case-count, case-007 guidance-bait) on classification only — safety checks still pass on those. Headline: the open-weight model holds the safety line as well as (and on the judgment checks slightly better than) the proprietary baseline, conservatively, since real-case ground truth is partly Gemini-derived. Artifact: `eval/results/2026-06-20T07-07-25-043Z.md`. NOTE: Gemini free tier is too tight for iterative runs (RPD exhausts after a few full passes → 429s the retry can't outwait); enable pay-as-you-go on the key's project for clean reruns.
...
**What shipped — a READ-ONLY, fail-open, review-gated weekly capture.** The Tracking Report is a **tertiary aggregator**, so per CONTENT-STANDARDS §2/§3.4 and AGENTS.md ("bind figures to primary sources, not the newsletter") it must **never** auto-write curated fields — unlike the PHAC/NWSS wastewater writers, whose sources are registered Tier 1/2 *primary* feeds mapped verbatim. The pipeline instead auto-fetches the current issue each week, diffs it against the live signals, and surfaces a human-actionable GitHub issue only when a *new* issue outruns the signals; it closes itself when the signals catch up. Fetch method per user choice: **plain server-side fetch + UA + retry** (same pattern as `ingest-phac`), keeping Bright Data inside its news-only AI-enrichment policy (no policy/audit changes).

**Files touched:**
- `scripts/ingest-tracking-report.mjs` — NEW. Discovers the current issue from the stable Brown landing page (`/our-work/tracking-report`), fetches it, parses the dateline, and diffs vs `signals.json`: per-signal mentioned/stale-vs-issue (compared to humans-only `lastChecked`), plus a best-effort uncovered-topic lexicon scan (watch-only, false-positive-tolerant). Writes only `ingest-tracking-report-result.json` (gitignored). Never touches `src/data`. Fail-open on any network/parse error (always exits 0).
- `scripts/test-ingest-tracking-report.mjs` — NEW. Offline unit tests (no network) covering URL discovery, date parse + slug fallback, signal matching/staleness, uncovered-topic scan, and the new-issue / caught-up / date-unparsed result paths.
- `.github/workflows/ingest-tracking-report.yml` — NEW. Weekly cron (Fri 14:00 UTC) + `workflow_dispatch`; `contents: read`, `issues: write`. Runs the capture and opens/updates/auto-closes a reused `tracking-report` GitHub issue. Stateless → bumpable to daily by editing one cron line. Transient fetch failures fail open (logged, no issue).
- `scripts/review-digest.mjs` — registered `tracking-report-capture` in `MONITOR_ARTIFACTS` so the daily Human Review Digest folds in its status.
- `package.json` — added `ingest:tracking-report` + `test:ingest-tracking-report` scripts.
- `.gitignore` — ignore `ingest-tracking-report-result.json`.

**Data-capture finding:** all 18 topics in the June 11 issue already map to existing signals (19 total; only Bay-Area rotavirus is dashboard-only). First live run: current issue **2026-06-11**, 18/19 signals mentioned, **0 stale** (all signals `lastChecked` 06-12/06-13, i.e. re-verified *after* the issue → caught up). Two uncovered-topic watch hits (diphtheria, rabies) confirmed as false positives (DTP/LASSARAB *vaccine* mentions, not outbreaks). No curated edits were warranted; future drift now surfaces automatically.

**License finding (no change needed):** the dual-license is already fully reconciled. AGPL-3.0-or-later (code) + CC BY-SA 4.0 (content) agree across `LICENSE`, `LICENSE-DATA.md`, `COMMERCIAL-LICENSE.md`, `NOTICE`, `CITATION.cff`, `package.json`, the README §License, and the in-app About page. Remaining `MIT` strings are intentional forward-only relicense history in `HANDOFF.md`/`MONITORING.md` and an unrelated TTS table in `PODCAST-EXPORT-DESIGN.md`.

**Verify:** `npm run ingest:tracking-report` (prints issue + caught-up status; writes the artifact); `npm run test:ingest-tracking-report`; full cadence `test:validators` + `validate:data` + `audit:ai-enrichment` + `audit:autonomy` + `build` all PASS. `review:digest` lists `tracking-report-capture` as an OK monitor. To go live: merge; first Friday run (or manual `workflow_dispatch`) reconciles the `tracking-report` issue.

## ✅ Timeline cleanup — dedup Andes duplicates, rebind 2 PHAC events, seed 5 coverage gaps (commit pending)

Curated `src/data/signal-timeline.json` cleanup, source-integrity-reviewed (NO-BLOCKER; session marker written by `source-integrity-agent`). Three parts:

1. **Dedup (removed 4):** the Andes hantavirus signal had been authored twice (a descriptive `tl-andes-*` pass + a date-stamped `t-2026*` pass), producing same-day twin cards. Removed the redundant twin in each pair, keeping the more detailed / correctly-sourced one: `t-20260519-ecdc-update` (kept `tl-andes-2026-05-19-ecdc-update`, correctly sourced to `ecdc-andes-surveillance`), `tl-andes-2026-05-08-cdc-han528` (kept `t-20260508`), `tl-andes-2026-05-06-ecdc-rra` (kept broader `t-20260506`), `tl-andes-2026-05-04-who-don599` (kept `t-20260504`). Every milestone still retains ≥1 event; no orphan refs.
2. **Rebind (2):** `t-20260517-phac-confirmation` and `t-20260516-canada` described PHAC/Canadian actions but cited `cdc-han-528` (a CDC advisory). Rebound to the existing `phac-andes-media-update` (Tier 2, PHAC).
3. **Coverage seed events (added 5):** the 2026-05-14 stub batch only seeded then-active signals, leaving 5 event-type signals with no timeline presence. Added one process-statement stub each ("Monitoring record added for … verify against <authority>"), bound to each signal's existing primary source: `yellow-fever-colombia-2026`→`paho-yellow-fever-2026`, `lassa-fever-2026`→`ncdc-lassa-fever`, `screwworm-onehealth-2026`→`usda-aphis-screwworm-status`, `pertussis-us-2026`→`cdc-pertussis-surveillance`, `fifa-world-cup-2026-prep`→`who-mass-gatherings`. No quantified claims; no new sources.

**Result:** timeline 46→47 events; signal coverage **8/19 → 13/19**; same-signal/same-day pairs 8→4 (the 4 remaining are legitimately distinct same-day events). The continuous wastewater/respiratory monitors (covid/noro/rota/rsv/hmpv/seasonal-flu) intentionally have no timeline events (trend dials, not chronologies). **Verify:** `npm run validate:data && npm run test:validators && npm run build` (PASS). **Deferred (see backlog):** rebinding `t-20260512-a` (mixed ECDC/Emory), `t-20260509` (Pathoplexus), `t-20260505` (Geneva) — no matching source registered; needs SME source decision.

## ✅ PHAC writer repointed to PHAC's CURRENT wastewater feed — Toronto/Vancouver go live (commit pending)

Completed the Canada integration: the previous PHAC writer targeted the retired `covidLive/wastewater/` path (frozen 2024-06), so Toronto/Vancouver correctly but unhappily showed "No current data". Verified PHAC's **current** National Wastewater Monitoring feed is live and recent, and repointed the writer to it. **Canada now publishes real data.**

Verified live before coding (`/src/data/wastewater/wastewater_trend.csv`): HTTP 200, parseable, newest `weekStart` **2026-05-31** (~15 days old → within the 45-day guard), covers Toronto (`city=="Toronto"`) and Vancouver (`city=="Metro Vancouver"`), pathogens covN2/fluA/fluB/rsv, categorical `Viral_Activity_Level` ∈ {Non-detect, Low, Moderate, High, NA2}.

Three improvements over the original brief (all reported):
- **Single-file fetch.** The trend table carries the level AND `weekStart`, so the flaky two-file (trend+aggregate) fetch is gone — one request, fewer failure modes.
- **PHAC's own city-level aggregate.** Rows include `grouping=="City"` (PHAC's metro rollup); we use it verbatim instead of self-maxing sites (site-max kept only as fallback). More faithful, less interpretation.
- **3-pathogen parity with NWSS** (`covN2→SARS-CoV-2`, `fluA→Influenza A`, `rsv→RSV`) — drops the old "COVID-only" caveat. `fluB` not surfaced.

Mapping (verbatim): `High→elevated/watch`; `Moderate`/`Low`/`Non-detect→normal/monitor`; `NA2`/blank/unmapped → skip. Severity still capped at `watch`; anti-stale guard (45d, per city) preserved on `weekStart`; Tier 2 `auto-phac`; fail-open; idempotent; Mexico unchanged ("Not monitored — never inferred"); NWSS unchanged.

**Files touched:**
- `scripts/ingest-phac-host-cities.mjs` — rewritten for the current schema (single trend file, `city`/`grouping`/`Viral_Activity_Level`/`weekStart`, 3 pathogens, city-aggregate-preferred rollup). Endpoints in configurable `PHAC_TREND_URL` (default = current path).
- `scripts/test-ingest-phac-host-cities.mjs` — rewritten against the real schema; reads new CSV fixtures.
- `scripts/fixtures/phac-wastewater-trend-current.csv`, `…-stale.csv` — NEW offline fixtures (current schema; current vs frozen-2024).
- `CONTENT-STANDARDS.md` §4.8 + `scripts/generate-status.mjs` (`status.json` guardrails) — updated for the current endpoint / single-file / 3-pathogen / city-aggregate reality.

**Verify:** `npm run test:ingest-phac && npm run test:ingest-nwss && npm run test:validators && npm run validate:data && npm run build` (PASS). Live proof: trigger `Ingest PHAC Host-City Wastewater` (`workflow_dispatch`) — Toronto/Vancouver respiratory tiles then show real SARS-CoV-2 / Influenza A / RSV wastewater status (dated 2026-05-31). **Coverage: 13/16 cities with current live data** (11 US + Toronto + Vancouver); 3 Mexico cities have no source. **Backlog:** if PHAC's feed later stalls, the guard auto-reverts the affected city to "No current data"; `fluB` surfacing + NWSS metro-county scoping remain optional.

## ✅ PHAC (Canada) host-city ingestion + Mexico "no source" qualifiers (commit pending)

Built the Canada/PHAC ingestion path and made the Mexican cities' unmonitored state explicit. User question was "is there a reason we can't get Canada and Mexico working?" — investigated and found: **Canada's writer is doable** (PHAC publishes machine-readable categorical CSVs), **Mexico genuinely is not** (no centralized national feed). **Important caveat discovered on first live run:** PHAC's publicly-accessible `covidLive/wastewater/` feed is **frozen at 2024-06-23** (historical file ends there; the trend file is undated). The first PHAC run therefore briefly published a 2024-vintage "elevated" reading — caught immediately and fixed by adding an **anti-stale guard** to BOTH writers (NWSS + PHAC): if the newest data point is older than `MAX_DATA_AGE_DAYS` (default 45), the observation is skipped (→ honest "No current data") rather than published as current. Net result: **Toronto/Vancouver correctly show "No current data" (PHAC source registered, but the feed has no current data)** — not stale data dressed as live. So live *current* coverage is 11/16 (US/NWSS); Canada is "source registered, feed dormant"; Mexico is "no source exists". The PHAC writer will publish real levels automatically if/when PHAC resumes a current feed.

**Canada / PHAC writer** (`scripts/ingest-phac-host-cities.mjs`):
- Live source is PHAC's wastewater **trend** CSV (`covid19-wastewater-trend.csv`), which carries PHAC's OWN categorical `latestLevel` (Low/Medium/High/New) + `latestTrend` per site. We map `latestLevel` verbatim (High→elevated, Low/Medium→normal, New/unmapped→skip) — same anti-fabrication discipline as CDC. The continuous `viral_load` file is deliberately NOT used (turning a raw load into a status = inventing thresholds = §7.2 human judgement).
- Matched by PHAC `region` (== city displayName: Toronto, Vancouver); highest level across the metro's sites; observation **dated from PHAC's historical main CSV** so freshness self-heals.
- **SARS-CoV-2 only** (PHAC flu/RSV are a separate feed, not yet wired — documented follow-on). Tier 2 source `phac-nwmp`, `provenance: auto-phac`, severity capped at `watch`. Same publish policy (`PHAC_PUBLISH_POLICY` auto|staged|off), fail-open, idempotent, one-time-approval model as NWSS.
- Verified with trend+main fixtures: dry-run + write-mode produced valid live observations (Toronto elevated / Vancouver normal in the fixture), `validate:data` PASS, then reverted (real data comes from CI).

**Mexico qualifiers:** rewrote `sourceCoverageSummary` for all 3 Mexican cities (Mexico City, Guadalajara, Monterrey) to state explicitly that **no official public wastewater/biosurveillance data source exists** and the city **cannot be monitored** from authoritative open data — reaffirming "Not monitored, never inferred". (No Mexican federal categorical wastewater feed comparable to CDC NWSS / PHAC; monitoring there is academic/decentralized.)

**Files touched:**
- `scripts/ingest-phac-host-cities.mjs`, `scripts/test-ingest-phac-host-cities.mjs` — NEW (writer + offline suite).
- `.github/workflows/ingest-phac-host-cities.yml` — NEW (weekly Sunday cron, staggered after the NWSS Saturday run; shared `host-city-ingest` concurrency group so the two writers serialize on the data file).
- `src/types.ts` — `provenance` union += `'auto-phac'`.
- `scripts/validate-data.mjs` — generalized the auto-observation contract into `HOST_CITY_AUTO_CONTRACTS` (per-provenance id prefix + `maxTier`: nwss Tier 1, phac Tier 2).
- `scripts/test-validate-data.mjs` — 3 auto-phac regression cases (also fixed: sentinel-date fixtures).
- `src/data/host-city-biosurveillance.json` — Mexico ×3 `sourceCoverageSummary` (curated text; content-standards discipline applied inline — change strengthens the no-inference posture).
- `.github/workflows/ci.yml` — added `test:ingest-phac`.
- `scripts/generate-status.mjs` + `public/status.json` — registered `phac-host-city-ingest` in `automation.dataWriters` (now 6 writers).
- `CONTENT-STANDARDS.md` — §4.8 (Canada/PHAC contract). `package.json` — `ingest:phac`, `test:ingest-phac`. `.gitignore` — `ingest-phac-result.json`.

**Verify:** `npm run test:ingest-phac && npm run test:validators && npm run validate:data && npm run build` (PASS). The PHAC `workflow_dispatch` runs successfully but writes **no** observations while PHAC's feed stays frozen (the guard skips the 2024 data) — Toronto/Vancouver show "No current data" with the PHAC source chip. **Backlog (Canada live data):** locate PHAC's *current* respiratory-virus wastewater endpoint — `covidLive/wastewater/` appears retired; the active respiratory-virus dashboard likely publishes elsewhere. Once found, point `PHAC_TREND_URL`/`PHAC_MAIN_URL` at it (one-line config) and Canada goes live with no other change. **Other backlog:** PHAC influenza/RSV (separate feed); optional metro-county scoping for the NWSS rollup.
...
Key design decisions (documented in CONTENT-STANDARDS §4.7):
- **Live dataset is `atcp-73re`** ("CDC Wastewater Viral Activity Level for SARS-CoV-2, Influenza A and RSV"). The spec's candidate `2ew6-ywp6` is now **archived** — confirmed live via the Socrata catalog. Columns: `state_territory`, `pathogen_target`, `week_end`, `site_wval_category`. All dataset config is in one block at the top of the script for easy maintenance.
- **Publish policy `NWSS_PUBLISH_POLICY` (auto | staged | off)**, default `auto`, set in the workflow. `auto` publishes deterministic Tier-1 data directly because re-keying official categorical values adds latency, not safety. `staged` keeps the spec's G21 review gate; `off` is a kill switch. This is a deliberate, scoped exception to §3.4 humans-only writes.
- **Severity capped at `watch`** — auto data raises a domain to "elevated" and a city to "watch" but never trips concern/action (those still require human curation or multi-source convergence).
- **State-level rollup** = highest category among a state's NWSS sites for the latest week, faithfully labelled in the summary (not a claim to reproduce CDC's own aggregate).
- **Self-healing**: if the job stops, observations age to "stale" via existing freshness logic then drop — stale data never shows as current. Fail-open on CDC outage; idempotent (replaces only `auto-nwss` obs, never curated); US-only.

**Files touched:**
- `scripts/ingest-nwss-host-cities.mjs` — NEW. The deterministic writer (pure transform fns exported for testing; `--dry-run` + `NWSS_FIXTURE_PATH` for offline runs).
- `scripts/test-ingest-nwss-host-cities.mjs` — NEW. Offline unit suite (category mapping, latest-week max rollup, id format, severity cap, US-only, idempotency). `npm run test:ingest-nwss`.
- `.github/workflows/ingest-nwss-host-cities.yml` — NEW. Weekly (Sat 12:00 UTC) cron + manual dispatch; ingest → validate → test → regenerate status/API → bot-commit with rebase-retry push.
- `src/types.ts` — `HostCityObservation.provenance?: 'curated' | 'auto-nwss'`.
- `scripts/validate-data.mjs` — auto-nwss contract enforcement (Tier 1 source, `auto-nwss-` prefix, respiratory/wastewater, severity ≤ watch, date required).
- `scripts/test-validate-data.mjs` — 5 auto-nwss regression cases.
- `CONTENT-STANDARDS.md` — §4.7 (auto-ingested host-city observations).
- `scripts/generate-status.mjs` + `public/status.json` — registered `nwss-host-city-ingest` in `automation.dataWriters`.
- `package.json` — `ingest:nwss`, `test:ingest-nwss`.
- `.gitignore` — `ingest-nwss-result.json`.

**Verify:** `npm run test:ingest-nwss && npm run test:validators && npm run validate:data && npm run build` (all PASS). Dry-run plan + a fixture write (then reverted) confirmed valid observations with `publicDisplayAllowed:true`, severity `watch`, Tier 1 `cdc-nwss` source. To go live: merge the PR (sets policy `auto`); first scheduled Saturday run (or a manual `workflow_dispatch`) publishes real levels — verify on `/host-cities` (US tiles show Normal/Elevated wastewater status). **Network note:** the dev sandbox has no outbound network, so the real Socrata fetch only runs in CI / on a networked machine; the transform is fixture-tested offline.

## ✅ NWSS ingestion spec + reconcile stranded host-city wiring (commit pending)

PR #20 merged only the base host-city feature (commit b40742a + SHA backfill 443b979). Two follow-up commits — the header nav rename "Host Cities" → "FIFA 2026", and the CDC NWSS + PHAC source-wiring (11 US cities → `cdc-nwss`, Toronto/Vancouver → `phac-nwmp`, Mexico → none) — were left on `feat/host-city-biosignals` and never reached main (`git branch --contains` confirmed they were not ancestors of the merge). This branch cherry-picks both onto current main and adds the option-2 implementation spec.

**Files touched:**
- `docs/HOST-CITY-NWSS-INGESTION.md` — NEW. Self-contained brief for another agent to build CDC NWSS Socrata-API ingestion: deterministic, Tier-1-bound, writes `provenance:"auto-nwss"` observations as `publicDisplayAllowed:false` (staged for human review via review-digest gate G21), mirroring the news→timeline auto-writer contract (CONTENT-STANDARDS §4.6).
- (cherry-picked) `src/components/NavBar.tsx` — nav label "FIFA 2026"; `src/data/signal-sources.json` — +`phac-nwmp` (Tier 2); `src/data/host-city-biosurveillance.json` — sourceIds wired; `src/pages/HostCityBioSignals.tsx` — clickable source links on cards.

**Verify:** `npm run validate:data && npm run test:validators && npm run build` (PASS); `/host-cities` nav reads "FIFA 2026"; the 11 US cities show "No current data" + clickable CDC NWSS links; Toronto/Vancouver cite PHAC; the 3 Mexican cities show "Not monitored".
...
**Design (load-bearing):**
- **Per-city status is derived, never stored.** The data file holds only city identity + source-backed observations; per-domain columns, overall severity, confidence, and source-freshness are computed by `src/utils/hostCityBioSignals.ts` from `publicDisplayAllowed:true` observations. Consequence: an "elevated city with no backing source-backed observation" is **structurally impossible**, not merely validated against. This collapsed the three highest adversarial-review findings (false-convergence, public/private leak, overclaim) into one structural fix.
- **Source registry reused** (`signal-sources.json`) — no forked registry; tier discipline inherited. Observations reference a resolvable `sourceId` + a deep-link `sourceUrl`.
- **Parent signal decoupled:** `fifa-world-cup-2026-prep` severity is NOT modified by this layer, so a curated host-city escalation cannot silently trip the news→timeline auto-promoter (fires at severity ≥ concern) or the high-severity RSS feed.
- **Three distinct empty states:** `Normal` (source-backed, quiet) / `No current data` (source registered, none current) / `Not monitored` (no source — dashed), so absence of signal is never misread as absence of threat.

**Source rules:** only authoritative/institutional/public source-backed observations; no fabricated sources, no inferred coverage, no observation without a verifiable public `sourceUrl`. `publicDisplayAllowed:false` = staged/under review — excluded from the public surface AND every derivation; surfaced for action via review-digest G21.

**Validation requirements** (`validate-data.mjs` host-city block; 11 regression tests in `test-validate-data.mjs`) — fails if: a public observation lacks `sourceId`/`sourceUrl`; a public observation lacks `reportDate`/`sampleDate` (freshness underivable); an observation references an unknown source or the wrong host city; `parentSignalId` or a city `sourceIds` entry doesn't resolve; any domain/status/severity/confidence/country enum is invalid; observation ids are not globally unique; `reportingLagDays ≠ reportDate − sampleDate`. The file is OPTIONAL (graceful-absent) so CI stays green before the layer is populated.

**Files touched:**
- `src/data/host-city-biosurveillance.json` — NEW. 16 host cities, zero observations (curated; in hook SENSITIVE_PATHS + CONTENT-STANDARDS §3.4).
- `src/types.ts` — NEW interfaces (`HostCityRecord`, `HostCityObservation`, `HostCityBiosurveillance`, `DerivedHostCityStatus`) + status type aliases.
...
- `.claude/hooks/check-source-integrity.sh`, `CONTENT-STANDARDS.md` §3.4 — curated-field registration.

**Review:** `content-standards-agent` — no blockers; 3 recommendations applied (§3.4 row, "Source confidence" label, disclaimer→`/resources` link). `source-integrity-agent` review truncated mid-run; registry-reuse / no-fabrication / attribution-enforcement confirmed directly and corroborated by the content-standards pass.
...
**Files touched:**
- `src/data/signals.json` — figure + `lastChecked` corrections (curated, humans-only fields)
- `src/data/signal-sources.json` — `lastVerified` → 2026-06-12 for the 7 re-verified primary sources

**Corrected (figures moved):**
- **ebola-bundibugyo-drc-2026** — DRC 321→**515** cases, 48→**91** deaths, CFR 15→**17.7**%; Uganda 9→**19** cases, 1→**2** deaths; removed the now-unstated DRC-suspected (116) metric; summary rebound to WHO DON606 (data as of 6 Jun 2026; combined 534/93, overall CFR 17.4%, ≥17 recovered).
- **measles-us-2026** — 1,983→**2,073** cases (as-of May 28 → June 11, 2026); 30 outbreaks & 40 jurisdictions unchanged; evidence "9 of 1,983"→"10 of 2,073" (CDC measles data).
- **avian-influenza-h5-2026** — metric "(no deaths)"→"**(1 death)**" and `currentSituation` "no deaths"→"one reported death" (CDC now reports one US H5 death); count 71 unchanged.
- **andes-hantavirus-mv-hondius-2026** — summary split "(11 confirmed, 2 probable)"→"**(12 confirmed, 1 probable)**" per ECDC (as of 11 Jun); total 13 + 3 deaths unchanged.
- `lastChecked` → 2026-06-12 on the four above **plus mpox-africa-clade-i-2026 / chikungunya-2026 / pertussis-us-2026** (posture re-verified live; no figure change).

**Held — binding unresolved, deliberately NOT changed (need a sourcing decision):**
- **yellow-fever-colombia-2026** — the "Tolima 153 cases / 62 deaths" metric is **not supported by its cited PAHO 13-Mar-2026 alert** (that alert states Tolima 25 cases/13 deaths for 2026 EW1–7; WHO DON570 gives Tolima 52/18 as of Apr 2025; the "121/47 cumulative" appears only in secondary trackers). `lastChecked` left at 2026-06-05.
- **cholera-africa-2026** — the "40,707 (Jan–May 14 2026)" figure could not be verified; the Africa CDC EI Weekly is a non-rendering PDF (only the 2025 cumulative 73,806/490 is searchable). `lastChecked` left at 2026-06-02.
- 10 surveillance / no-hard-claim signals (wastewater cluster, FluView, Lassa, screwworm, FIFA) were not re-pulled this pass; `lastChecked` unchanged — they surface individually via the review-digest as designed.

**Verify:** `npm run validate:data` (OK) + `npm run test:validators` (OK); the next scheduled (or manually re-run) `Status Refresh` should pass `verify:production` because `lastOfficialSourceCheck` is now 2026-06-12 (<168h). The gate threshold (168h) was left **unchanged** per the user's decision to keep the gate as-is. `audit:sources` failures (`usda-aphis-screwworm-status` timeout; `cdc-han` known-blocked 403) are pre-existing/transient and unrelated to this change.
...
`scripts/enrich-news.mjs` now classifies each Gemini failure: a credential/authorization denial (HTTP 401/403, `PERMISSION_DENIED`, `UNAUTHENTICATED`, `API_KEY_INVALID`/"API key not valid", "denied access") → `failureClass: 'auth-denied'`; everything else → `'transient'`. The classification is written to the internal `ai-news-enrichment-result.json` artifact (and echoed in the run log: `Gemini enrichment unavailable (auth-denied)`). The workflow's "Reconcile AI news brief issue" step now early-returns when `result.failureClass === 'auth-denied'`, so it stops commenting. The denial remains fully visible in the workflow run logs and the result artifact; nothing about the data pipeline or fail-open behavior changes.

**Policy:** compliant with `AI-ENRICHMENT-POLICY.md` (no structured-field writes, fail-open preserved, server-side only, news-only) and `audit:ai-enrichment` stays green. The `[AI BRIEF]` issue is maintainer-facing, not the public dashboard, so the §"Prohibited Uses" no-diagnostics-on-the-public-dashboard rule is unaffected.

**Files touched:**
- `scripts/enrich-news.mjs` — added `classifyGeminiFailure()`; emit `failureClass` + log it in the Gemini-failure fallback path.
- `.github/workflows/update-news.yml` — "Reconcile AI news brief issue" step early-returns on `failureClass === 'auth-denied'`.

**Verify:** `node --check scripts/enrich-news.mjs`; classifier covered by an inline unit check (403/401/400-INVALID → auth-denied; 429/503/timeout/no-text → transient); end-to-end run with a bogus key produced `400 API_KEY_INVALID` → `failureClass: "auth-denied"` with `news.json` untouched; `npm run audit:ai-enrichment` → OK. Live: after the next scheduled run, issue #6 should receive **no** new "Gemini denied" comment while the 403 persists. (Existing issue #6 is left open as the tracker; restoring Gemini access makes the next successful run comment "enrichment OK" on it again.)
...
User asked: "verify that based on the signals, no additional briefings are needed. if new ones are needed, create them and logic to auto create them in the future based on signal additions or changes." Audited first: every one of the 17 current signals already has at least one section matching the briefings picker priority list, so zero new briefings need to be "created." More importantly, briefings turn out to be a **derived view** of signals (no separate `briefings.json`), so "new briefings" never meant separate content — it meant "ensure each signal has a section the briefings view can surface." Flagged the only halt-condition option (LLM-generated briefing content) and confirmed with the user that the right interpretation was: extend the picker to use the EMS-specific section IDs that already exist in data, AND add a validator that fails the build if any future signal ships without a priority section. No new content was generated by any AI — every section already in `signals.json` is preserved and surfaced as-is.

Two concrete improvements landed:

**1. Centralized picker.** `Briefings.tsx` and `DemoPack.tsx` each had their own slightly-different `SECTION_PRIORITY` array — maintenance hazard. Both now import `pickBriefingSection` from a single shared `src/utils/briefings.ts`. The canonical priority list lives in `src/utils/briefings-priority.json` and is the only source of truth.

**2. Priority list extended to surface existing EMS-specific sections.** The previous picker priority list ignored sections like `ems-transport-protocols` (which `ebola-bundibugyo-drc-2026` has) and `ems-surge-readiness` (which `fifa-world-cup-2026-prep` has). The new list is most-EMS-specific → most-generic: `ems-specific`, `ems-transport-protocols`, `ems-surge-readiness`, `operational-guidance`, `protocols-and-guidance`, `clinical-management`, `clinical-profile`, `ppe-and-ipc`, `ppe-and-isolation`, `infection-prevention`. Observable effect: Ebola's briefing card now surfaces "EMS transport protocols" instead of generic "Operational guidance"; other 16 signals continue to surface the same section they did before.

**3. Validator coverage check.** `scripts/validate-data.mjs` now reads `briefings-priority.json` at startup and fails if any signal has zero matching sections in its `detailSections`. Error message names the priority list and points to the config file. Regression test added to `scripts/test-validate-data.mjs` (`signal-without-briefings-priority-section`) that mutates a signal's section IDs to non-priority values and confirms the validator fails with the expected message. This means: every future signal added to `signals.json` is forced to have at least one operationally-actionable section before it can ship, with no manual "create a briefing" step.

**Files touched:**
- `src/utils/briefings-priority.json` — new. Single source of truth for the priority list (shared with the validator). Includes a `_comment` field explaining the contract.
- `src/utils/briefings.ts` — new. Exports `BRIEFING_SECTION_PRIORITY`, `pickBriefingSection(signal)`, and `hasBriefingCoverage(signal)`. Docstring includes an explicit "do NOT auto-generate section content from signal data" warning so future agents do not re-litigate the halt condition.
- `src/pages/Briefings.tsx` — removed local `SECTION_PRIORITY` array + `pickBriefingSection`; imports from `../utils/briefings`. Also dropped now-unused `SignalDetailSection` type import.
- `src/pages/DemoPack.tsx` — same treatment. `EMS World Briefing` page now shares the picker with `/briefings`, removing the inconsistency between the two surfaces.
- `scripts/validate-data.mjs` — loads `briefings-priority.json` at startup; adds per-signal coverage check inside the existing `detailSections` validation block.
- `scripts/test-validate-data.mjs` — new regression test confirming the validator fails when a signal lacks any priority section.
- `HANDOFF.md` — this entry + timestamp.

**What was NOT changed (deliberately):**
- No new dependencies, no AI/LLM integration, no content generation. `AI-ENRICHMENT-POLICY.md` is unchanged. `CONTENT-STANDARDS.md` curated-fields-are-humans-only rule is preserved.
- No edits to `src/data/signals.json` or any other curated data file.
- No change to FIFA's signal-specific picker in `DemoPack.tsx` (`ems-surge-readiness` → `surveillance-planning` → first) — that is intentional FIFA-specific logic distinct from the general briefings picker.

**Verify:** `npm run test:validators` (passes, includes new regression test); `npm run validate:data` (passes); `npm run build` (passes); `/briefings` → confirm ebola card shows "EMS transport protocols" section title (was "Operational guidance"); to confirm the new rule is enforced, temporarily rename a signal's `operational-guidance` section id to `epidemiology` in `src/data/signals.json` and run `npm run validate:data` — should fail with "detailSections lacks any briefings-priority section."
...
- **Auto-ingest more signals from reachable structured feeds (medium–large; the durable fix for the recurring source-review cadence).** Deferred from the 2026-06-20 168h→336h threshold change. Goal: cut how many signals need manual re-attestation by extending the existing deterministic Tier-1/2 auto-ingest pattern (`scripts/ingest-nwss-host-cities.mjs`, `scripts/ingest-phac-host-cities.mjs`) from host-city wastewater to **national signals**. That pattern's invariants must hold: verbatim mapping (no AI/derived numbers), severity capped at `watch`, fail-open, anti-stale guard, `auto-*` provenance, auto-`lastChecked` attestation on write, registered in `status.json` `dataWriters`, and a weekly workflow — only deterministic Tier-1/2 **primary** feeds may auto-write curated fields; anything needing interpretation stays human-gated (CONTENT-STANDARDS §3.4).
  - **Machine-fetchable candidates (worth building):** CDC NWSS national wastewater via the Socrata API on `data.cdc.gov` (same source family the host-city writer already uses) → `covid-wastewater` / `rsv-wastewater` / `candida-auris-wastewater` / `norovirus-wastewater`; CDC FluView API → `seasonal-influenza`; WastewaterSCAN data API/CSV (`data.wastewaterscan.org`) → `norovirus` / `rotavirus` / `hmpv`. Biggest single win: the **CDC-NWSS national writer** (covers ~4 wastewater signals at once).
  - **NOT auto-ingestable — stay manual (primaries are prose/PDF/SPA, confirmed unreachable via WebFetch 2026-06-20):** ebola (WHO DON), cholera (Africa CDC), mpox (WHO), lassa (NCDC PDF sitreps), pertussis (CDC qualitative only), chikungunya / yellow-fever (PAHO), screwworm (USDA), andes (ECDC). measles + H5 are CDC static HTML data pages (no clean API) — scrape-with-care or keep manual.
  - **Effort:** ~0.5–1 day per source family (writer + offline test suite + CONTENT-STANDARDS §4.x contract subsection + validator rules + `dataWriters` registration + weekly workflow), mirroring the NWSS/PHAC builds. This is the real path to "hands-off" — it removes the auto-ingestable signals from the human cadence entirely; the prose/PDF-sourced signals remain a (now 14-day) manual review.

- **Timeline source-rebind, deferred subset (small, needs SME/source decision).** Three Andes timeline events still cite `cdc-han-528` but describe non-CDC actions, and no matching source is registered for them: `t-20260512-a` (lead is an ECDC epidemiological update — could rebind to `ecdc-andes-surveillance`, but the event also folds in the Emory negative result, so it is mixed-authority), `t-20260509` (Pathoplexus / Virological.org sequence analysis — no source registered), `t-20260505` (Geneva University Hospitals Swiss-case confirmation — no source registered). To resolve: either register the appropriate Tier-2/3 sources (Pathoplexus/Virological, Geneva HUG) then rebind, or split `t-20260512-a` into its ECDC and Emory components. Routed through `source-integrity-agent` + `evidence-binding-agent`. Low urgency — the displayed content is accurate; only the source chip attribution is imperfect.
