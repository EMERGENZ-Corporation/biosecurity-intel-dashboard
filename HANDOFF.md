...
User asked to clear the degraded state without hiding stale data. This pass re-reviewed the 13 stale signals listed in `public/status.json` against official primary sources, updated only source-confirmed facts and map details, regenerated public artifacts, and added durable refresh classifications so future agents can tell which signals are auto-writable, auto-checkable with human review, or manual-only.

**Files touched:**
- `src/data/signals.json` - added required `refreshClassification` to all signals; updated `lastChecked` only for reviewed stale signals; refreshed source-confirmed facts for Andes hantavirus, Lassa fever, yellow fever, screwworm, pertussis, mpox, cholera, wastewater-tracked signals, Candida auris, chikungunya, and FIFA mass-gathering preparedness.
- `src/data/signal-sources.json` - refreshed `lastVerified` only for official sources actually reviewed this session; corrected the NCDC Lassa source to the situation-report index and clarified the PAHO yellow-fever note.
- `src/types.ts` and `scripts/validate-data.mjs` - added/validated `SignalRefreshClassification` and exposed refresh-classification counts in status summaries.
- `src/data/signal-discovery-policy.json` - extended the signal-discovery lane into a per-signal source-currency matrix with explicit auto-write, human-review, and manual-only handling.
- `scripts/generate-status.mjs` and `scripts/review-digest.mjs` - surfaced refresh-classification counts and included the classification in stale/watch issue payloads.
- `.github/workflows/weval-baseline.yml` - restored monthly `cron: '0 5 1 * *'` and made Weval execution secret-aware so missing eval keys skip safely instead of disabling the schedule.
- `CONTENT-STANDARDS.md` - documented the refresh classifications and the rule that source-currency prevention must not mask stale data.
- `public/status.json` and `public/api/v1/*` - regenerated status, API, RSS, news, and timeline artifacts from the refreshed source data.

Notable source-confirmed updates:
- Andes hantavirus now reflects ECDC's 17 June 2026 update: 13 cases, including 12 confirmed and 1 probable, with 3 deaths and very low EU/EEA public risk.
- Lassa fever now reflects NCDC epidemiological week 23: 855 confirmed cases, 214 confirmed-case deaths, 23 affected states, and 109 affected LGAs.
- Yellow fever now uses the PAHO regional alert baseline: 34 cases and 15 deaths across Bolivia, Colombia, Peru, and Venezuela in epidemiological weeks 1-7 of 2026.
- Screwworm no longer asserts unsupported New Mexico/Zavala markers; the map keeps APHIS-confirmed U.S. detections and uses La Salle County, Texas for the June 9 APHIS news item.
- Cholera, wastewater trends, pertussis counts, mpox/chikungunya country burdens, and other unsupported fine-grained claims were neutralized where the accessible official source body did not confirm the prior values.

**Result:** `public/status.json.status` is `ok`, `signals.staleSignalIds` is empty, and generated signal/API/RSS artifacts now share the same refreshed matrix. This does not hide remaining source-work: `audit:sources` and `audit:source-drift` still fail on broader registry drift, older secondary sources, and a USDA APHIS timeout, so those remain honest backlog items rather than being papered over with fresh dates.

**Verify:** `npm.cmd run validate:data`, `npm.cmd run test:validators`, `npm.cmd run test:signal-discovery`, `npm.cmd run generate:status`, `npm.cmd run generate:api`, `npm.cmd run build`, and `npm.cmd run audit:autonomy` all PASS. Direct status/API check confirms `status=ok`, `staleSignalIds=[]`, 21 API signals. (The refresh-classification split reported here — manual-only 6 / auto-checkable-human-reviewed 9 / auto-writable 6 — was superseded by the same-day pre-commit review pass below, which reclassified all six `auto-writable` signals to `auto-checkable-human-reviewed` because no deterministic writer script exists in the repo yet.)

## ✅ Official signal discovery matrix refresh (commit pending)

User asked for hands-off current-source monitoring and specifically called out FDA Cyclospora, Uganda filovirus/Marburg-vs-Bundibugyo disambiguation, Powassan virus, and State Department/U.S. Embassy Kampala monitoring. This pass adds the missing Cyclospora and Powassan signals, updates the Bundibugyo Ebola signal to explicitly include DRC & Uganda with an Embassy Kampala operational sentinel, and ships a daily deterministic discovery workflow that emits internal `signal-candidates.json` proposals instead of letting operational alerts overwrite epidemiologic facts.

**Files touched:**
- `src/data/signals.json` - added `cyclospora-us-2026` and `powassan-us-2026`; updated `ebola-bundibugyo-drc-2026` with DRC/Uganda naming, Kampala/Wakiso geography, Embassy Kampala sentinel marker/source linkage, and a Marburg disambiguation guard.
- `src/data/signal-sources.json` - registered CDC cyclosporiasis surveillance, FDA CORE/foodborne Cyclospora sources, CDC Powassan current-year/data/about pages, and U.S. Embassy Kampala alert feed/direct health alert as official operational sources.
- `src/data/signal-discovery-policy.json` - new allowlist/max-severity/source-role policy for the discovery pipeline, including `canWriteCounts:false` for Embassy/State operational sentinels.
- `scripts/discover-signals.mjs` + `scripts/test-signal-discovery.mjs` - new deterministic scanner and parser/safety tests for FDA Cyclospora, CDC Powassan, WHO Bundibugyo vs Marburg, and Embassy RSS/WordPress fallback behavior.
- `.github/workflows/signal-discovery.yml`, `.github/workflows/ci.yml`, `scripts/audit-autonomy.mjs`, `scripts/generate-status.mjs`, `package.json`, `.gitignore` - daily workflow, CI/audit/script/status registration, and ignored internal artifact.
- `CONTENT-STANDARDS.md` - added section 4.9 defining official operational sentinels and signal-discovery guardrails.
- `public/status.json` and `public/api/v1/*` - regenerated public status/API artifacts after signal/source additions.

**Verify:** `npm.cmd run test:signal-discovery`, `npm.cmd run test:validators`, `npm.cmd run validate:data`, and `npm.cmd run build` all PASS. `npm.cmd run discover:signals` PASSes and writes internal `signal-candidates.json`; in this sandbox the Embassy feed/WP/direct fallbacks report fetch warnings but remain internal source-lane health. The separate local Weval cron disablement noted in this entry is resolved by the follow-up degraded-state refresh above, and `npm.cmd run audit:autonomy` now PASSes.

**Follow-up (same day) — pre-commit review pass:** a full multi-agent review (grant-claims, ai-enrichment-policy, content-standards, architecture, source-integrity, evidence-binding, security-posture) flagged that the workflow name **"Autonomous Signal Discovery"** and `public/status.json`'s `automation.mode: "autonomous-with-review-gates"` overstated what the pipeline actually does (it stages internal candidates and opens a human-review issue; it writes zero public or structured fields). Independently, `signal-discovery-policy.json` declared `canAutoWriteStructuredFacts:true` / `auto-writable` for six signals (`cyclospora-us-2026`, `powassan-us-2026`, `pertussis-us-2026`, `seasonal-influenza-2026`, `covid-wastewater-2026`, `rsv-wastewater-2026`) and `canWriteCounts:true` + `maxAutoSeverity:"action"` on two WHO source lanes, with **no deployed writer script anywhere in the repo** to exercise that authority. Fixed all of it before commit:
- Renamed the workflow to **"Official Signal Discovery"** (`.github/workflows/signal-discovery.yml`, `scripts/generate-status.mjs` `dataWriters[].workflow`, issue-footer text, label description) and dropped the unused `actions: read` permission (no step reads the Actions API).
- Changed `automation.mode` from `autonomous-with-review-gates` to **`assisted-pipeline-with-review-gates`** in `scripts/generate-status.mjs`, `src/data/signal-discovery-policy.json`, and the two guard checks in `scripts/validate-data.mjs` / `scripts/audit-autonomy.mjs` that pinned the old string.
- Flipped `cyclospora-us-2026`, `powassan-us-2026`, `pertussis-us-2026`, `seasonal-influenza-2026`, `covid-wastewater-2026`, and `rsv-wastewater-2026` from `refreshClassification: "auto-writable"` / `canAutoWriteStructuredFacts:true` to `"auto-checkable-human-reviewed"` / `false` in both `src/data/signals.json` and `src/data/signal-discovery-policy.json` — `auto-writable` remains a valid schema value in `src/types.ts` for whenever a tested writer is actually built, but no signal claims it today.
- Set `who-bundibugyo-situation` and `who-disease-outbreak-news` source lanes to `maxAutoSeverity:"watch"` / `canWriteCounts:false`, matching CONTENT-STANDARDS §4.9's "auto-published discoveries are capped at watch" rule and the already-correct hardcoded behavior in `discover-signals.mjs` for that signal.

**Verify (after fixes):** `npm.cmd run validate:data`, `npm.cmd run test:validators`, `npm.cmd run test:signal-discovery`, `npm.cmd run generate:status`, `npm.cmd run generate:api`, `npm.cmd run build`, and `npm.cmd run audit:autonomy` all PASS; `status.automation.mode` reads `assisted-pipeline-with-review-gates` and `byRefreshClassification` shows `auto-writable: 0`.
